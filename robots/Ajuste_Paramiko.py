"""
robots/Ajuste_Paramiko.py — RPA Suite v6.0
==========================================
Robot de ajustes de inventario (NC), migrado a Paramiko.
...
class SSHSessionManager:
    ...
    def __init__(self, host: str, port: int, user: str, password: str, trazador: Trazador):
        ...
        self.transcript_file = str(log_dir / f"debug_terminal_ajuste_{int(time.time())}.txt")
...
def ejecutar_ajuste_paramiko(df, log_func, progress_func, archivo_origen):
    """Robot de Ajuste/NC usando Paramiko."""
    trazador = Trazador("AJUSTE_PARAMIKO", archivo_origen, total_filas=len(df), log_func=log_func)
    trazador.iniciar()
    ...
    try:
        ...
        for i, fila in df_datos.iterrows():
            ...
            try: 
                delta = str(int(float(fila.iloc[1])))
            except Exception: 
                continue
            ...
            titulo_log = f"SKU {sku} — {desc_m[:25]} (Delta: {delta})" if desc_m else f"SKU {sku} (Delta: {delta})"
            ...
            with trazador.etapa("CARGA_ITEM", titulo_log, fila=i + 1, sku=sku):
                ...
                if es_gramaje:
                    ssh.send('g'); ssh.send(delta); ssh.send_enter()
                    ssh.send(dato_d); ssh.send_enter()
                else:
                    ssh.send('u'); ssh.send(delta); ssh.send_enter()

                if ssh.channel.recv_ready(): ssh.channel.recv(4096)
                if progress_func: progress_func((i + 1 - 3) / total_filas)
        ...
        log_func(f"🔥 Error en robot Ajuste Paramiko: {e}")
